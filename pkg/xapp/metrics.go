...
//
//-----------------------------------------------------------------------------
type MetricGroupsCacheRegisterer interface {
...
type MetricGroupsCache struct {
	...
	reg          MetricGroupsCacheRegisterer
}

func (met *MetricGroupsCache) Registerer(reg MetricGroupsCacheRegisterer) {
	met.reg = reg
}

func (met *MetricGroupsCache) cReg(metric string) Counter {
	if met.reg != nil {
		met.combineCounterGroupsWithPrefix("", met.reg.RegisterCounters([]CounterOpts{CounterOpts{Name: metric, Help: "Amount of " + metric + "(auto)"}}))
...
func (met *MetricGroupsCache) gReg(metric string) Gauge {
	if met.reg != nil {
		met.combineGaugeGroupsWithPrefix("", met.reg.RegisterGauges([]CounterOpts{CounterOpts{Name: metric, Help: "Amount of " + metric + "(auto)"}}))
